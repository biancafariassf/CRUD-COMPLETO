...
const PacotesCrud = () => {
  ...
  const handleCriarPacote = async () => {
    try {
      await firestore().collection('pacotes_viagem').add({ companhia_aerea, duracao, hotel_pct, pagantes, valor, cod_pct });
      setCompanhia('');
      setDuracao('');
      setHotel('');
      setPagantes('');
  ...
  const handleEditarPacote = async () => {
    ...
    try {
      await firestore().collection('pacotes_viagem').doc(editando.id).update({ companhia_aerea, duracao, hotel_pct, pagantes, valor, cod_pct  });
      setCompanhia('');
      setDuracao('');
      setHotel('');
      setPagantes('');
